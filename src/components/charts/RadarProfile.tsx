...
export function RadarProfile({
  ...
  const option = useMemo<EChartsOption>(() => {
    const names = axes.map((axis) => t(`dimensions.${axis.dimension}`))
    ...
    return {
      ...
      radar: {
        ...
        indicator: axes.map((axis) => ({
          name: t(`dimensions.${axis.dimension}`),
          max: 100,
          min: 0,
        })),
        axisName: {
          ...
          formatter: (name?: string) => {
            if (!name) return ''
            const axis = axes.find((item) => t(`dimensions.${item.dimension}`) === name)
      ...
      {view === 'chart' ? (
        ...
        <div className="overflow-x-auto px-4 py-3">
          <table className="w-full min-w-[420px] border-collapse text-[13px]">
            ...
            <tbody>
              {axes.map((axis) => (
                <tr key={axis.dimension} className="border-b border-line/60 last:border-0">
                  <td className="py-2 pr-3">{t(`dimensions.${axis.dimension}`)}</td>
