...
export function AnalysisHome() {
  ...
  return (
    <>
      ...
      <Link to="/analyse/jahr" className="mt-4 flex min-h-14 items-center justify-between gap-3 border border-line bg-surface px-4 py-3 hover:bg-accent-quiet">
        <span className="flex items-center gap-3">
          <CalendarRange size={18} className="text-ink-muted" aria-hidden />
          <span>
            <span className="block text-[14px] font-medium">{t('year.title', { year: new Date().getUTCFullYear() })}</span>
            <span className="block text-[12px] text-ink-secondary">{t('year.intro')}</span>
          </span>
        </span>
        <ArrowRight size={16} className="text-ink-muted" aria-hidden />
      </Link>

      <Link to="/hinweise" className="mt-4 flex min-h-14 items-center justify-between gap-3 border border-line bg-surface px-4 py-3 hover:bg-accent-quiet">
        <span className="flex items-center gap-3">
          <Lightbulb size={18} className="text-ink-muted" aria-hidden />
          <span>
            <span className="block text-[14px] font-medium">{t('hints.title')}</span>
            <span className="block text-[12px] text-ink-secondary">{t('hints.summarySub')}</span>
          </span>
        </span>
        <ArrowRight size={16} className="text-ink-muted" aria-hidden />
      </Link>

      <Link to="/belastung" className="mt-4 flex min-h-14 items-center justify-between gap-3 border border-line bg-surface px-4 py-3 hover:bg-accent-quiet">
        <span className="flex items-center gap-3">
          <Activity size={18} className="text-ink-muted" aria-hidden />
          <span>
      ...
      {/* Das Sportmodul steht hier und nicht in der Navigation: Es ist eine
          Vertiefung der Analyse, keine eigene Ebene. Hinter der Stufe Elite
          — der Gate an der Route sagt es, wenn sie fehlt. */}
      <Link to="/sportmodul" className="mt-4 flex min-h-14 items-center justify-between gap-3 border border-line bg-surface px-4 py-3 hover:bg-accent-quiet">
        <span className="flex items-center gap-3">
          <Gauge size={18} className="text-ink-muted" aria-hidden />
          <span>
            <span className="block text-[14px] font-medium">{t('sportModule.title')}</span>
            <span className="block text-[12px] text-ink-secondary">{t('sportModule.eyebrow')}</span>
          </span>
        </span>
        <ArrowRight size={16} className="text-ink-muted" aria-hidden />
      </Link>
