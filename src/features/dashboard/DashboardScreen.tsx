...
export function DashboardScreen() {
  ...
  const current = useMemo(
    () =>
      radarProfile(data.results, mode, new Date(), data.profile.disciplineId),
  ...
  const summaries = useMemo(
    () => toSummaries(data.results, locale),
    [data.results, locale],
  );
  ...
  const athlete = {
    ...
    bodyWeightKg:
      data.biometrics.find((b) => b.bodyWeightKg != null)?.bodyWeightKg ?? null,
  ...
  if (data.results.length === 0) {
    ...
    return (
      <EmptyState
        ...
        body={
          discipline ? t("dashboard.emptyBody") : t("dashboard.noSportHint")
        }
        action={
        ...
        <div>
          ...
          <p className="mt-1.5 text-[13px] text-ink-secondary">
            ...
            <span className="readout">
              {formatDate(lastAssessment, locale)}
            </span>
            <span className="text-ink-muted">
              {" "}
              ({formatRelativeMonths(lastAssessment, locale)})
            </span>
          </p>
        </div>
      ...
      {!discipline && (
        ...
        >
          <span className="min-w-0">
            <span className="label-tag text-accent-text">
              {t("dashboard.noSportTitle")}
            </span>
        ...
        <Panel className="lg:col-span-2">
          <PanelHeader
            ...
            subtitle={t("dashboard.strongestIs", {
              dimension: strongest
                ? t(`dimensions.${strongest.dimension}`)
                : "—",
            })}
            action={
              <Button asChild variant="ghost" size="sm">
                  ...
                  <span className="mt-0.5 block text-[11px] leading-snug text-ink-muted">
                    {t("dashboard.scoreBasis", {
                      tests: score.basis
                        .map(
                          (b) =>
                            getTest(b)?.shortName[locale] ?? t(`metrics.${b}`),
                        )
                        .join(" + "),
                    })}
                  </span>
          ...
          <Panel className="lg:col-span-2">
            ...
            <div className="px-2 py-3">
              <TrendChart
                points={trend.points}
                unit={
                  trend.test.primaryUnit === "s" ? "s" : trend.test.primaryUnit
                }
                locale={locale}
