...
export function DecisionRow({ locale }: { locale: AppLocale }) {
  ...
  const axes = useMemo(() => radarProfile(data.results, 'population', new Date(), data.profile.disciplineId), [
    data.results,
  ...
  const biggestGain = useMemo(() => {
    const rows = baselineComparisons(data.results).filter((r) => r.changePercent != null)
