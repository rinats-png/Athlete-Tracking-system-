        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center justify-between gap-2">
            <span className="text-[16px] font-medium">
              {main ? <Link to={`/sport/${main.id}`} className="inline-flex min-h-11 min-w-11 items-center hover:underline">{main.name[locale]}</Link> : '—'}
            </span>
            ...
            <ul className="mt-1 flex flex-wrap gap-2">
              {additional.map((d) => (
                <li key={d.id} className="flex items-center gap-1 border border-line pl-3 text-[13px]">
                  <Link to={`/sport/${d.id}`} className="inline-flex min-h-11 min-w-11 items-center hover:underline">{d.name[locale]}</Link>
