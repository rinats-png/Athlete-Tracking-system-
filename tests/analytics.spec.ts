...
test.describe("Erste gegen letzte Messung", () => {
  ...
  test("ein Test mit nur einer Messung erscheint nicht", () => {
    expect(
      baselineComparisons([result("back_squat_1rm", "2026-01-01", 150)]),
    ).toEqual([]);
  });
...
test.describe("Terminvergleich", () => {
  const data: AthleteData = {
    ...
    results: [
      result("back_squat_1rm", "2026-01-10", 150, {
        id: "r1",
        assessmentId: "a1",
      }),
      result("back_squat_1rm", "2026-04-10", 165, {
        id: "r2",
        assessmentId: "a2",
      }),
      result("cooper_12min", "2026-01-10", 3000, {
        id: "r3",
        assessmentId: "a1",
      }),
      result("standing_broad_jump", "2026-04-10", 2.4, {
        id: "r4",
        assessmentId: "a2",
      }),
    ],
...
test.describe("Belastbarkeit", () => {
  ...
  test("jede Komponente ist mit ihrem Beleg nachvollziehbar", () => {
    const { components } = confidenceScore(
      [
        result("back_squat_1rm", "2026-04-20", 160),
  ...
  test("eine alte Messung senkt die Aktualität, ohne sie zu verwerfen", () => {
    const alt = confidenceScore(
      [result("back_squat_1rm", "2025-01-01", 160)],
      asOf,
    );
  ...
  test("die Gesamtzahl ist das Mittel der offengelegten Anteile", () => {
    ...
    const mean =
      components.reduce((sum, c) => sum + c.value, 0) / components.length;
...
test.describe("Analyse-Bildschirm", () => {
  test("zeigt Belastbarkeit, Ausgewogenheit und Terminvergleich", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Analyse", level: 1 }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("meter", { name: "Abdeckung", exact: true }),
    ).toBeVisible();
    await expect(page.getByRole("meter", { name: "Aktualität" })).toBeVisible();
    await expect(
      page.getByRole("meter", { name: "Datenqualität" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("columnheader", { name: "Veränderung" }).first(),
    ).toBeVisible();
    await expect(
      page.getByText(/Messungen, \d+ Tage, R²/).first(),
    ).toBeVisible();
  ...
  test("ohne Messungen wird nichts ausgewertet statt eine leere Tabelle zu zeigen", async ({
    page,
  }) => {
